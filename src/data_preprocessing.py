...
def process_telemetry(telemetry, failures, machines, errors, maintenance):
    ...
    failures_by_machine = (
        failures.groupby("machineID")["datetime"].apply(list).to_dict()
    )
    ...
    maintenance = maintenance.sort_values(["machineID", "datetime"]).reset_index(
        drop=True
    )
    ...
    print("\nGenerated Nan's:")
    print(
        (telemetry[maint_cols].isna().mean() * 100)
        .round(2)
...
def main():
    ...
    plt.savefig("data/graphs/sensor_distribution.png")
    ...
    fail_rate_by_model = (
        telemetry.groupby("model")["will_fail_in_24h"]
        .mean()
        .sort_values(ascending=False)
    )
    print("\nFail rate of each machine model")
...
def sanity_check_nans(telemetry):
    ...
    print("\nGenerated Nan's:")
    print(telemetry[["will_fail_in_24h", "will_fail_in_48h"]].isnull().sum())
